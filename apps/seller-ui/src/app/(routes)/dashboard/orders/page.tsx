...
const Orders = () => {
  ...
  const orders = mockOrders.orders as Order[];
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: any) => (
          <span className='font-semibold text-slate-900'>
            {currencyFormatter.format(row.original.totalPrice)}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        header: 'Date',
        cell: ({ row }: any) => {
          const date = new Date(row.original.createdAt).toLocaleDateString('en-IN');
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className='border-b border-slate-100'>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className='p-3 text-left font-semibold text-slate-500 uppercase tracking-wider text-xs bg-slate-50/55'
                  >
